...
feature propagation across adjacent and near-adjacent tokens. These
...
and compare the manuscript's structural profile against $13$ natural-language comparators and a shuffled-token control. Five findings
...
We compare Voynich against $13$ natural-language comparators:
$2$ historical literary texts (Middle English Chaucer, Gutenberg~\#22120;
KJV English, Gutenberg~\#10900) and $10$ Wikipedia-derived corpora from the
Leipzig Corpora Collection (Turkish, Hungarian, Finnish, Hebrew, Arabic,
Latin, North Azerbaijani, Italian, Estonian, Swahili, Swahili; all \texttt{wikipedia\_2021\_100K}
editions). A shuffled-token control (\texttt{Gibberish}) is derived from the
...
\begin{figure}[ht]
  ...
  \caption{Prefix vs.~suffix self-clustering across $15$ tested systems.
...
\begin{table}[ht]
  ...
  and Swahili (Leipzig 100K) both tested SYMM-LOW.}
  \label{tab:pstable}
  \begin{tabular}{lrrrl}
    \toprule
...
suffix $0.96\times$, ratio $0.82$). Having bidirectional morphology in
a natural language does not suffice to produce Voynich's symmetric
elevated clustering.
...
languages is larger than our $13$-language sample, and a language with
...
\begin{enumerate}
  ...
  \item \textbf{Bidirectional self-clustering symmetry.} $P/S = 0.99$ with
        both prefix and suffix SC $> 1.5\times$. Unique among $13$ tested
        systems.
...
broad classes and the comparator pool to $13$ natural languages; other natural-language types (Sinitic,
...
\paragraph{8.~Cross-transcription validation incomplete.} Results are
stable across EVA-based transliterations but have not been tested with
non-EVA alphabets that assign different character boundaries.
...
\begin{enumerate}
  \item \textbf{Cross-transcription validation.} Define equivalent token
        families under FSG and Currier alphabets. Test whether the
        bidirectional symmetry and transition rules survive.
  ...
  \item \textbf{Additional language types.} Sinitic (isolating morphology),
        polysynthetic languages (Nahuatl, various Indigenous American
        languages), and historical shorthand systems (Pitman, Tironian
        notae) have not been tested and represent distinct structural
        profiles not covered by the current comparator set.
\end{enumerate}
